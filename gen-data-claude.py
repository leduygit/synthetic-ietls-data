...
def retry_claude_call(func: Callable, max_retries: int = 3, delay: float = 2.0, *args, **kwargs) -> Any:
...
def generate_essay(prompt: str, band: int, feedback: str = None, current_essay: str = None, current_score: float = None, target_band: float = None) -> str:
    """Generate essay using Claude with dynamic token allocation for 250-300 words."""
...
IMPORTANT: Write in PLAIN TEXT only. Do NOT use any markdown formatting, bold text (**), italics (*), headers (#), or special formatting. Write as a normal IELTS essay would appear on paper."""
    ...
    try:
        return retry_claude_call(call_claude, max_retries=3, delay=2.0, 
...
def score_essay(essay: str, question: str) -> Dict[str, float]:
    """Score an IELTS essay using Claude and band descriptions."""
    band_descriptions = load_band_descriptions()
...
Please score this essay according to IELTS criteria."""
    try:
        response = retry_claude_call(call_claude, max_retries=3, delay=2.0,
...
Please provide specific change recommendations to exactly reach Band {target_band}."""
    try:
        feedback = retry_claude_call(call_claude, max_retries=3, delay=2.0,
...
def main():
    parser = argparse.ArgumentParser(description='Generate synthetic IELTS essays using Claude')
    ...
    # Check API key
    if not anthropic_api_key:
        print("Error: Anthropic API key required. Set ANTHROPIC_API_KEY environment variable in .env file")
        sys.exit(1)
    # Load data
